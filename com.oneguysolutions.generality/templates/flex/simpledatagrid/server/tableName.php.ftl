...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `${tableName}` (<#list columns as column><#if !column.primaryKey>${column.columnName}<#if column != lastCol>,</#if></#if></#list>) VALUES (%s,%s)" ,			GetSQLValueString($_REQUEST["nombre_usuario"], "text"), # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `usuario` WHERE id_usuario = %s",
		GetSQLValueString($_REQUEST["id_usuario"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `usuario` SET nombre_usuario = %s,contrasena = %s WHERE id_usuario = %s", 
			GetSQLValueString($_REQUEST["nombre_usuario"], "text"), 
			GetSQLValueString($_REQUEST["contrasena"], "text"), 
			GetSQLValueString($row_recordset["id_usuario"], "int")
		);
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `usuario` WHERE id_usuario = %s", 
			GetSQLValueString($row_recordset["id_usuario"], "int")
		);
