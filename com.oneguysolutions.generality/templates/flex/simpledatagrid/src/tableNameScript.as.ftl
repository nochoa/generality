 ...
 */ 
private var fields:Object = {<#list columns as column><#if sqlNumberTypes?seq_contains(column.dataType)>'${column.columnName}':Number<#elseif sqlStringTypes?seq_contains(column.dataType)>'${column.columnName}':String</#if><#if column != lastCol>,</#if></#list>};
/**
...
<#list columns as column>
  <#if column.primaryKey>
    if(e.dataField == "${column.columnName}Col")
    {
        e.preventDefault();
        return;
    }
...
private function editCellEnd(e:DataGridEvent):void
{
    ...
    var parameters:* =
    {<#list columns as column>
        "${column.columnName}": vo.${column.columnName}Col<#if column != lastCol>,</#if>
    </#list>}
...
private function insertItem():void {
    var parameters:* =
    {
     ...
     <#list columns as column>
       <#if !column.primaryKey>
        "${column.columnName}": ${column.columnName}Col.text<#if column != lastCol>,</#if>
       </#if>
    </#list>};
...
public function deleteHandler(e:*):void
{
    ...
    {
        var id_usuario:Number = parseInt(e.data.toString(), 10);
        for (var index:Number = 0; index < dataArr.length; index++)
        {
            if (dataArr[index].id_usuarioCol == id_usuario)
            {
