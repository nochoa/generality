...
<?php
require_once(dirname(__FILE__) . "/defaultconn.php");
 ...
 */
$filter_field = "${opt.keyColumn.columnName}";

/**
 * we need to escape the value, so we need to know what it is
 * possible values: text, long, int, double, date, defined
 */
$filter_type = "<@opt.insertPhpType opt.keyColumn />";
...
function findAll() {
	 ...
	 */
	$fields = array(<#list columns as column>'${column.columnName}'<#if column != lastCol>,</#if></#list>);

	$where = "";
	if (@$_REQUEST['filter'] != "") {
		$where = "WHERE " . $filter_field . " = " . GetSQLValueStringForSelect(@$_REQUEST["filter"], $filter_type);	
	}
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `${tableName}` $where"); 
	...
	$query_recordset = "SELECT <#list columns as column>${column.columnName}<#if column != lastCol>,</#if></#list> FROM `${tableName}` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `${tableName}` (<@opt.insertColumns />) VALUES (<@opt.insertValues />)" , <@opt.insertParameters />);
	...
	if ($ok) {
		...
		$toret = array(
			"data" => array(
				array(<#list columns as column>
					<#if column.primaryKey>"${column.columnName}" => mysql_insert_id(), 
					<#else>"${column.columnName}" => $_REQUEST["${column.columnName}"]<#if column != lastCol>,</#if>#
					</#if> 
				</#list>)
			), 
			"metadata" => array()
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `${tableName}` WHERE ${opt.keyColumn.columnName} = %s",
		GetSQLValueString($_REQUEST["${opt.keyColumn.columnName}"], "<@opt.insertPhpType opt.keyColumn />")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `${tableName}` SET <#list columns as column><#if !column.primaryKey>${column.columnName} = %s<#if column != lastCol>,</#if></#if></#list> WHERE ${opt.keyColumn.columnName} = %s", 
			<#list columns as column><#if !column.primaryKey>GetSQLValueString($_REQUEST["${column.columnName}"], "<@opt.insertPhpType column />"),</#if></#list> 
			GetSQLValueString($row_recordset["${opt.keyColumn.columnName}"], "<@opt.insertPhpType opt.keyColumn />")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"${opt.keyColumn.columnName}" => $row_recordset["${opt.keyColumn.columnName}"],
						<#list columns as column> 
						"${column.columnName}" => $_REQUEST["${column.columnName}"]<#if column != lastCol>,</#if>#</#list>
					)
				), 
				"metadata" => array()
			);
...
function delete() {
	...
	if ($num_rows > 0) {
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["${opt.keyColumn.columnName}"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
